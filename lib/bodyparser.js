...
module.exports = {
    ...
    parse: function(req, cb){
        ...
        req.on('data', function(chunk){
            ...
            try{
                while(text){
                    endPos = text.indexOf(sep)
                    if (-1 === endPos) break
                    switch(pt){
                    case PT_CHANNEL:
                        orders.push(text.substring(0, endPos)) 
                        pt = PT_HEADER
                        break
                    case PT_HEADER:
                        head = JSON.parse(text.substring(0, endPos))
                        orders.push(head)
                        body = []
                        pt = (head.len > 0) ? PT_BODY : PT_HEADER
                        break
                    case PT_BODY:
                        body.push(text.substring(0, endPos))
                    ...
                    text = text.substr(endPos+sepLen)
                }
            }catch(exp){
                return error(exp)
            }
