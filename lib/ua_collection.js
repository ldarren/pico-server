...
MeasurementProtocol.prototype = {
    ...
    dimension: function(id, val){
        if ('number' !== typeof(id) || id < 1 || id > 200 || !val){
            console.error('analytics: invalid custom dimension', id, val);
            return this;
        }
    ...
    metric: function(id, val){
        if ('number' !== typeof(id) || id < 1 || id > 200 || !val || 'number' !== typeof(val)){
            console.error('analytics: nvalid custom metric', id, val);
            return this;
        }
    ...
    pageview: function(page, title, host){
        if (!page){
            console.error('analytics: pageview without page');
            return this;
        }
    ...
    event: function(category, action, label, value){
        if (!category || !action){
            console.error('analytics: event trigger without category or action', category, action);
            return this;
        }
        var params = {
            t: 'event',
    ...
    transaction: function(transactionId, affiliation, revenue, shipping, tax, currency){
        if (!transactionId){
            console.error('analytics: transaction without transactionId');
            return this;
        }
    ...
    item: function(name, price, quantity, sku, category, currency){
        ...
        if (!ti || !name){
            console.error('analytics: itemHit without transactionId or item name', ti, name);
            return this;
        }
        var params = {
            t: 'item',
    ...
    social: function(network, action, target){
        if (!network || !action || !target){
            console.error('analytics: social hit without network or action or target', network, action, target);
            return this;
        }
        var params = {
            t: 'social',
