...
parseJson = function(req, res, cb){
    ...
    req.on('end', function(){
        ...
        try{
            ...
            data = json = undefined;
            if (orders) cb(req, res, orders, now);
...
if(session)session.log(res.connection.remoteAddress, res[PICO_CHANNEL_ID] || NO_CHANNEL, packet);
    res.end(packet);
...
Session.prototype.getModel = function(modelId){
    if (SESSION_REQ === modelId || SESSION_RES === modelId || SESSION_JOBS === modelId || SESSION_ACKS === modelId || SESSION_TIME === modelId)
