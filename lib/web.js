...
renderView = function(session, res, channelId, jobs, cb){
    ...
    if (channelId && channelStorage){
        channelStorage.storeMessage(channelId, JSON.stringify(data), function(err, resId){
            if (err) return cb(err);
            view.resId = resId;
...
resendRes = function(channelId, res, cb){
    if (!channelStorage) return cb();

    channelStorage.retriveAllMessages(channelId, function(err, msgs){
        if (err) return cb(err);
    });
},
...
function removeAck(session, order, next){
    ...
    channelStorage.removeMessage(order.data.resId, function(err){
        next(err);
    });
}
