...
console.log('orders:', orders);
        getChannelId(req, clientInfo.pathname, function(err, channelId){

            if (channelId) res.setHeader(PICO_CHANNEL, channelId);
            res.writeHead(200, {
            ...
            createSession(req, res, channelId, function(err, session){
                factory(session, orders, 0, function(err, order){
                    ...
                    resendRes(session, channelId, function(err){
                        res.end();
                    });
                });
            });
...
getChannelId = function(req, path, cb){
    var channelId = req.headers[PICO_CHANNEL];
    if (channelId) return cb(null, channelId);
    if ('/channel' === path){
