...
console.log('appKey:',order.key,'localKey:',crypto.createHmac('md5', secretKey+dt).update(JSON.stringify(order.data)).digest('base64'));
    return order.key === crypto.createHmac('md5', secretKey+dt).update(JSON.stringify(order.data)).digest('base64');
},
