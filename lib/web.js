...
// 2) remove pico-channel-id and pico-channel-pass
const
JOB_RENDER = 0,
JOB_MODEL_INFOS = 1,
JOB_OBJ = 2,
    ...
    req.on('end', function(){
        ...
        auth(orders, 0, function(err, order){
            ...
            getChannelId(req, clientInfo.pathname, function(err, channelId, channelPass){
                ...
                res.writeHeader(200, PICO_HEADERS); // always return 200 status code or client wont receive body
                createSession(req, res, channelId, function(err, session){
session.log(res.connection.remoteAddress, channelId, data);
                    data = undefined;
    ...
    if (modelInfos){
        ...
        for(i=0,l=modelInfos.length;i<l;i++){
            ...
            for(j=0,k=mis.length; j<k; j++){
                mi = mis[j];
                if (!mi || undefined === mi.modelId || undefined === mi.key) return this.error('Exception! @api:', api, 'modelId or key not found in', mi);
            }
        }
    }
...
Session.prototype.createModelInfo = function(modelId, key){
    if (undefined === this[modelId] || undefined === this[modelId][key]) return this.error('Exception!', modelId, 'and', key, 'Cant be found in session');
