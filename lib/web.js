...
web = {
    ...
    retainChannel: function(req, res){
        ...
        var channelId = res.getHeader(PICO_CHANNEL);
...
showView = function(session, cb){
    ...
    channelId = res.getHeader(PICO_CHANNEL);
...
function removeAck(session, order, next){
    ...
    var
    channelId = session[G_CCONST.SESSION_RES].getHeader(PICO_CHANNEL),
    resId = order.data.resId;
    channelStorage.removeMessage(channelId, resId, function(err){
        if (!err){
            var
