...
web = {
    ...
    retainChannel: function(req, res){
        ...
        var channel = res.getHeader(PRAGMA);
        if (!res || !res.writable || !channel) return false;
        var
        idpass = channel.split(' '),
...
if(session)session.log(res.connection.remoteAddress, res.getHeader(PRAGMA), view);
    res.write(JSON.stringify(view));
    if (delimiter) res.write(delimiter);
},
