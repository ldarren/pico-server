...
web = {
    ...
    keepChannel = function(req, res){
        ...
        var channelId = req.headers['pico-channel'];
...
console.log('orders:', orders);
        ...
        factory(session, orders, 0, function(err, order){
            ...
            var channelId = req.headers['pico-header'];
            if (connMap[channelId]) return;
            switch(clientInfo.pathname){
                case 'channel':
                    ...
                    channelStorage.allocateChannelId(function(err, channelId){
                        res.setHeader('pico-channel', channelId);
                        res.end();
                    });
                    break;
                default:
...
function removeAck(session, order, next){
    ...
    channelStorage.removeResponse(order.data.resId, function(err){
        next();
    });
}
