...
auth = function(order){
    var dt = order.date;
console.log('now:',Date.now(),'dt',dt,'cullAge',cullAge,'result',((Date.now() - dt) > cullAge));
    if ((Date.now() - dt) > cullAge) return false;
console.log('appKey:',order.key,'localKey:',crypto.createHmac('md5', secretKey+dt).update(JSON.stringify(order.data)).disgest('base64'));
    return order.key === crypto.createHmac('md5', secretKey+dt).update(JSON.stringify(order.data)).disgest('base64');
},
