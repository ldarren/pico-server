...
console.log('appKey:',order.key,'localKey:',crypto.createHmac('md5', secret+dt).update(JSON.stringify(order.data)).disgest('base64'));
    return order.key === crypto.createHmac('md5', secret+dt).update(JSON.stringify(order.data)).disgest('base64');
},
