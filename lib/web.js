...
PICO_HEADERS = {
    ...
    'Access-Control-Allow-Headers': 'X-Channel',//'X-Requested-With, Content-Type, Pragma',
...
onConnect = function(req, res){
    switch(req.method){
    case 'POST':
        for(var key in PICO_HEADERS){
            res.setHeader(key, PICO_HEADERS[key]);
        }
        break;
    case 'OPTIONS':
...
reply = function(req, res, orders, startTime){
    getChannelId(req, url.parse(req.url, true).pathname, function(err, channelId, channelPass){
        ...
        if (channelId) {
            res.setHeader(CHANNEL, ''+channelId+' '+channelPass);
