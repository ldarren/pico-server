...
web = {
    ...
    keepChannel: function(req, res){
        if (!channelStorage) return false;
        var channelId = req.headers[PICO_CHANNEL];
...
console.log('orders:', orders);
        createSession(req, res, function(err, session, channelId){
            factory(session, orders, 0, function(err, order){
                ...
                resendRes(channelId, session, function(err){
                    switch(clientInfo.pathname){
                        case 'channel':
                            ...
                            channelStorage.newChannelId(function(err, channelId){
                                if (err) session.log(err);
                                if (channelId)
                                    res.setHeader(PICO_CHANNEL, channelId);
                                res.end();
                            });
                            break;
                        default:
...
createSession = function(req, res, cb){
    var channelId = req.headers[PICO_CHANNEL];
    var session = new Session(req, res);
    if (channelId && channelStorage){
        channelStorage.retrieveMsgIds(channelId, function(err, acks){
            cb(err, new Session(req, res, acks), channelId);
        });
    }else{
        cb(null, new Session(req, res, null), channelId);
    }
},
...
renderView = function(session, res, channelId, jobs, cb){
    if (!jobs.length) return cb();
    var job = jobs.shift();
    if (!job) return renderView(session, res, channelId, jobs, cb);

    var 
    modelInfos = job[JOB_MODEL_INFOS],
    renderMode = job[JOB_RENDER];
    if (!renderMode || !modelInfos || !modelInfos.length || !modelInfos[0].length) return renderView(session, res, channelId, jobs, cb);
    ...
    if (channelId && channelStorage){
        channelStorage.newMessageId(channelId, function(err, resId){
            ...
            channelStorage.storeMessage(channelId, resId, packet, function(err){
                if (err) return cb(err);
                res.write(packet);
                return renderView(session, res, channelId, jobs, cb);
            });
        });
        return;
    }
...
showView = function(session, cb){
    var
    channelId = session[G_CCONST.SESSION_REQ].headers[PICO_CHANNEL];
    res = session[G_CCONST.SESSION_RES],
    jobs = session[G_CCONST.SESSION_JOBS];

    if (!res || !jobs) return cb('No res or no jobs at showView');

    renderView(session, res, channelId, jobs, cb);
},
resendRes = function(session, channelId, cb){
    if (!channelStorage) return cb();

    channelStorage.retriveMessages(channelId, session[G_CCONST.SESSION_ACKS], function(err, msgList){
        if (err) return cb(err);

        var res = session[G_CCONST.SESSION_RES];
        for(var i=1,l=msgList.length; i<l; i+=2){
            res.write(msgList[i]);
        }
    });
},
...
function removeAck(session, order, next){
    ...
    var resId = order.data.resId;
    channelStorage.removeMessage(resId, function(err){
        if (!err){
            var
