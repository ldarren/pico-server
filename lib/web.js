...
auth = function(order){
    ...
    if ((Date.now() - dt) > cullAge){
        console.warn('received old request',order.api,dt,str);
        return false;
    }

    var test = (order.key === crypto.createHmac('md5', secretKey+dt).update(str).digest('base64'));
    if (!test){
        console.warn('received invalid request',order.api,dt,str);
        return false;
    }
    return true;
},
