...
config  = {},
channelStorage = null,
secretKey = null, cullAge = 360000,delimiter = '',
web = {
...
auth = function(order){
    ...
    if (secretKey && !(order.key === crypto.createHmac('md5', secretKey+t).update(str).digest('base64'))){
...
//TODO: test for https or http here
    ...
    server.listen(libConfig.port, function(){
        ...
        delimiter = config.delimiter ? JSON.stringify(config.delimiter) : delimiter;
        ...
        console.log('web server listening to ', libConfig.port);
