...
PICO_HEADERS = {
    ...
    'Access-Control-Allow-Headers': 'X-Requested-With, Content-Type, Pragma',
...
web = {
    ...
    retainChannel: function(req, res){
        ...
        var channelId = res[PICO_CHANNEL_ID];
...
parseJson = function(req, res, cb){
    ...
    req.on('end', function(){
        ...
        try{
            ...
            data = json = undefined;
            if (orders) cb(req, res, orders);
            else res.setTimeout(DELAY_ERROR, dc, 400);
        }
    });
},
...
parseMultipart = function(req, res, contentType, cb){
    var
    json = {},
    data = '',
    boundary = parseContentType(contentType)['boundary'];

    if (!boundary) return res.setTimeout(DELAY_ERROR, dc, 400);

    req.on('data', function(chunk){
        req.pause();
        var end = data.length;
        data += chunk;
        var begin = data.indexOf(boundary, end);

        if (-1 === begin) return;
        data = data.substr(begin+boundary.length);

        var section = data.substr(0, boundary);
        if (!section) return;
...
onConnect = function(req, res){
    ...
    if (-1 === contentType.indexOf('multipart/form-data'){
        ...
        parseMultipart(req, res, contentType, reply);
    }
},
reply = function(req, res, orders){
    auth(orders, 0, function(err, order){
        if (err){
            console.error(res.connection.remoteAddress, err, data);
            // delay error to prevent ddos
            res.setTimeout(DELAY_ERROR, dc, 403);
            return;
        }

        getChannelId(req, url.parse(req.url, true).pathname, function(err, channelId, channelPass){

            if (channelId) {
                res.setHeader(PRAGMA, ''+channelId+' '+channelPass);
                res[PICO_CHANNEL_ID] = channelId;
            }
            createSession(req, res, channelId, function(err, session){
session.log(res.connection.remoteAddress, channelId);
                factory(session, orders, 0, function(err, order){
                    if (err) {
                        session.log(err);
                        return showErr(session, err, order, res);
                    }
...
getChannelId = function(req, path, cb){
    ...
    var
    channel = req.headers[PRAGMA],
    channelId, channelPass;
...
createSession = function(req, res, channelId, cb){
    if (channelId && channelStorage){
        channelStorage.retrieveMsgIds(channelId, function(err, acks){
            ...
            cb(null, new Session(req, res, acks));
        });
    }else{
...
pipeline = function(session, processList, processIdx, order, cb){
    ...
    process(session, order, function(err){
        if (err) {
            console.error(err);
            return cb(err.code ? err : G_CERROR['500'], session);
        }
...
auth = function(orders, orderIdx, cb){
    ...
    if (secretKey){
        var
        key = crypto.createHmac('md5', secretKey+t).update(data).digest('base64');
...
factory = function(session, orders, orderIdx, cb){
    ...
    api = order.api;
    if (!order || !api) return factory(session, orders, orderIdx, cb);
...
if(session)session.log(res.connection.remoteAddress, res[PICO_CHANNEL_ID], packet);
    res.end(packet);
...
session.log(res.connection.remoteAddress, channelId, packet);
        ...
        res.write(packet);
        if (delimiter) res.write(delimiter);
session.log(res.connection.remoteAddress, 'no channel', packet);
        cb();
    });
},
...
function removeAck(session, order, next){
    ...
    var
    channelId = session[SESSION_RES][PICO_CHANNEL_ID],
...
Session.prototype.getModel = function(modelId){
    if (SESSION_REQ === modelId || SESSION_RES === modelId || SESSION_JOBS === modelId || SESSION_ACKS === modelId)
