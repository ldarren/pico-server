...
module.exports = {
    ...
    ajax: function(method, href, params, headers, cb, userData){
        ...
        if (undefined === params.length){
            ...
            var opt = {tidy:1, mergeArr:1};
            for(var i=0,l=params.length; i<l; i++){
                data = mergeObj(data, params[i], opt);
            }
        }
        if (!data){
            console.error('ajax: params has to be an array of objects',params);
            return cb(null, null, userData);
        }

        var isGet = 'GET' === (options.method = method.toUpperCase());
        if (isGet) options.path += '?' + qs.stringify(data);
...
console.log(isGet, options, chunk);
        ...
        if (!isGet) req.write(qs.stringify(data));
        req.end();
    },
