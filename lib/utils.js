var
path = require('path'),
mergeObj = require('../../common/objTools').mergeObj,
loadLibs = function(keys, context, cb){
    ...
    key = keys.shift(),
    value = lib[key],
    module = require(context.root+'/lib/'+value.mod);
...
loadElements = function(elementPaths, context, cb){
    ...
    elements = require(context.root+'/app/'+elementPath);
    if (!elements){
...
exports.loadConfig = function(rootPath, baseConfig, envConfig){
    var config = mergeObj(baseConfig, envConfig);
    // root and config are reserved words in config object
    if (!config || !config.app || !config.lib || !config.app.elements) {
        console.error('not enough sections in config, config must contain app, lib and elements sections');
        return false;
    }
    if (!config || config.root || config.config) {
        console.error('config contain root or/and config reserved word sections');
        return false;
    }
